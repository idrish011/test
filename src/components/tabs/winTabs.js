...
export class winTabs extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            url:global.Ip + global.Port+"/winstory/getdumpbyid/"+sessionStorage.getItem("user_email")+'/'+idVal[0]
        });
        ...
        axios.get(global.Ip + global.Port + '/winstory/' + idVal[0], {
            ...
            .then(res => {
                ...
                // var a=global.Details.WINSTORY_OTHER_FILTER;
                // var other_filters = JSON.parse(res.data.WINSTORY_OTHER_FILTER);
                // console.log(other_filters);
                // var LOBValue = "";  //[]
                // if (other_filters.LOB[0] != null) {
                //         for (var l = 0; l <= other_filters.LOB.length - 1; l++) {
                //             LOBValue += other_filters.LOB[l].LOB_TYPE +"-"+ other_filters.LOB[l].LOB_NAME+"<br/>";
                //         }
                //         this.setState({
                //             LOBValue: LOBValue
                //         });
                ...
                if (global.Details.hasOwnProperty(prop)) {
                    ...
                    if (global.Details.LOB.length > 0) {
                        for (var s = 0; s <= global.Details.LOB.length - 1; s++) {
                            LOBValue += global.Details.LOB[s].FILTER_NAME + "<br/>";
                        }
                        this.setState({
                            LOBValue: LOBValue
                        });
                ...
                if ((global.Details.WINSTORY_THUMBNAIL != "") && (global.Details.WINSTORY_THUMBNAIL != undefined) && (global.Details.WINSTORY_THUMBNAIL != null)) {
                    dealInfoTab += "<img  class='auto-height' src=" + global.Details.WINSTORY_THUMBNAIL +'?'+new Date().getMilliseconds() +"> <br/>";
        ...
        axios.post(global.Ip + global.Port + '/admin/promote', reqParm, {
            ...
            axios.get(global.Ip + global.Port + '/winstory/' + idVal[0], {
                ...
                .then(res => {
                    ...
                    //         });
                    //     }
                    // global.Details.SALES_PLAY.length
      ...
      //  if()
        ...
        return (
            <div>
                ...
                <div className="mt-20 mb-20 Win-logo"><img src={imgLog +'?'+new Date().getMilliseconds()} />
                {sessionStorage.getItem('role').indexOf('winloader') != -1 && <span class="col-md-3"> <a href={this.state.url}><button type="button" class="btn btn-primary pull-right mb-20 ml-20 btn-sm"><i class="icon-Download"></i>Win Export</button></a></span>}
                    {/* {this.state.setWinStatus != "NA" && */}
                        <span class="col-md-3"><a href="javascript:void(0)" onClick={e => this.RequestDemo(e)} class="btn pull-right btn-sm btn-primary">Request Assistance</a></span>
                    {/* } */}
                        ...
                        <Tabs defaultActiveKey="Overview" id="uncontrolled-tab-example" className="mb-4">
                            ...
                            <Tab eventKey="Overview" title="Overview">
                                ...
                                <div class="row descAsset pd10 ">
                                    ...
                                    <div class="col-sm-6"><strong>Account Team</strong> <br />{this.state.Details.WINSTORY_REPS_SE} </div>
                                    <div class="col-sm-6"><strong>Win Quarter</strong><br />{this.state.Details.WINSTORY_FISCAL_QUARTER}</div>
                                    {this.state.LOBValue != "NA" && <> <div class="col-sm-6"><strong>LOB</strong><br /><span dangerouslySetInnerHTML={{ __html: this.state.LOBValue }}></span></div></>}
                                    ...
                                    {(this.state.Details.WINSTORY_REFERENCEABLE == "yes") ? (
                                        <div class="col-sm-6"> <strong>Referenceable Account</strong><br /> <span class="icon-Tick green" ></span></div>
                                    ):(
                                        <div class="col-sm-6"> <strong>Referenceable Account</strong><br /> <span class="icon-Close red" ></span></div>
                                    )

                                    }
                                   <div class="col-sm-6"><strong>Sales Lead</strong><br /> {this.state.Details.WINSTORY_SALES_LEAD}</div>
                                   <div class="col-sm-6"><strong>ECA/Engineer Lead</strong><br /> {this.state.Details.WINSTORY_ECA_ENGG_LEAD}</div>
                                   {sessionStorage.getItem('role').indexOf('winloader') != -1 && <>
                                   {((this.state.Details.WINSTORY_REFERENCE_STATUS != "")&&(this.state.Details.WINSTORY_REFERENCE_STATUS != null)) && <> <div class="col-sm-6"><strong>Reference Status</strong><br /> {this.state.Details.WINSTORY_REFERENCE_STATUS}</div></>}
                                   {((this.state.Details.WINSTORY_CUSTOMER_CONTACT != "")&&(this.state.Details.WINSTORY_CUSTOMER_CONTACT != null)) && <>    <div class="col-sm-6"><strong>Customer Contact</strong><br /> {this.state.Details.WINSTORY_CUSTOMER_CONTACT}</div></>}
                                   {((this.state.Details.WINSTORY_REFERENCE_PLAN != "")&&(this.state.Details.WINSTORY_REFERENCE_PLAN != null)) && <>      <div class="col-sm-6"><strong>Reference Plan</strong><br /> {this.state.Details.WINSTORY_REFERENCE_PLAN}</div></>}
                                   {((this.state.Details.WINSTORY_REFERENCE_PLAN != "")&&(this.state.Details.WINSTORY_REFERENCE_ASSET != null)) && <>        <div class="col-sm-6"><strong>Reference Assets</strong><br /> {this.state.Details.WINSTORY_REFERENCE_ASSET}</div></>}
                                   {((this.state.Details.WINSTORY_CUSTOMER_LIFECYCLE != "")&&(this.state.Details.WINSTORY_CUSTOMER_LIFECYCLE != null)) && <>      <div class="col-sm-6"><strong>Customer Lifecyle</strong><br /> {this.state.Details.WINSTORY_CUSTOMER_LIFECYCLE}</div></>}
                                   {((this.state.Details.WINSTORY_REFERENCE_LANGUAGE_INCLUDED != "")&&(this.state.Details.WINSTORY_REFERENCE_LANGUAGE_INCLUDED != null)) && <>       <div class="col-sm-6"><strong>Was reference lanuage included in the contract?</strong><br /> {this.state.Details.WINSTORY_REFERENCE_LANGUAGE_INCLUDED}</div></>}
                                   {((this.state.Details.WINSTORY_PROPOSED_GOLIVE_DATE != "")&&(this.state.Details.WINSTORY_PROPOSED_GOLIVE_DATE != null)) && <>      <div class="col-sm-6"><strong>What is the proposed Go-Live date?</strong><br /> {this.state.Details.WINSTORY_PROPOSED_GOLIVE_DATE}</div></>}
                                   {((this.state.Details.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET != "")&&(this.state.Details.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET != null)) && <>     <div class="col-sm-6"><strong>What is the first consumption milestone usage target date?</strong><br /> {this.state.Details.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET}</div></>}
                                   </>
                                    }
                                   </div>
                                    ...
                                    <>
                                        <div class="row descAsset pd10 ">
                                            {((this.state.Details.WINSTORY_CONSULTING_Q1 !=null) && (this.state.Details.WINSTORY_CONSULTING_Q1.trim() !="") && (this.state.Details.WINSTORY_CONSULTING_Q1 !=undefined)  ) &&
                                                <div class="col-sm-6"><strong>Why the Customer Bought/ The Impact of the purchase to the customer </strong><br /> {this.state.Details.WINSTORY_CONSULTING_Q1}</div>}
                ...
                <div id="requestdemo" class="modal" role="dialog">
                    ...
                    <div class="modal-dialog hide" id="requestForDemoStatus">
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title">Request Assistance Status</h4>
                            </div>
                            <div class="modal-body">
                                <Alert show={this.state.show} variant="light">
                                    <Alert.Heading><small> Assistance request saved and email notification sent successfully.</small></Alert.Heading>
