...
// console.log(likeNamesplit);
            ...
            if(res.data.sharecount<1){
                var shareCount='Be first to share Asset';
            }else{
                var shareCount= res.data.sharecount;
            }
      ...
      openSocial = (e) => {
        if (document.getElementById('ico-social').classList.contains("hide")) {
            document.getElementById('ico-social').classList.remove('hide')            // code...
          }
          else{
            document.getElementById('ico-social').classList.add('hide')            // code...
      ...
      sendMail = (e) => {
        ...
        global.IDAsset=ID[1];
        // var resparam=  {
        //     contentid:global.IDAsset,
        //     contenttype:"asset"
        // }
        axios.post(global.Ip + global.Port + '/user/captureactivity/w/share/' + sessionStorage.getItem("user_name") + '/' + sessionStorage.getItem("user_email"),{},{ headers: {
            user_email: sessionStorage.getItem("user_email"),         
            contentid:global.IDAsset,
            contenttype:"asset"
        }})
        .then(response => {
            window.open('mailto:?subject=Asset reference from AssetHub &body=Please explore the asset '+window.location.href);
  ...
  this.setState({likeImg:likeImg});
    ...
    //   sendMail = (e) => {
    //     axios.post(global.Ip + global.Port + '/user/captureactivity/w/share/' + sessionStorage.getItem("user_name") + '/' + sessionStorage.getItem("user_email"))
    //     .then(res => {
    ...
    // }
      sendslack = (e) => {
          window.open();
        // axios.post('https://slack.com/api/chat.postMessage').then(res => {
      ...
      handleCommenteMangement = (e) => {
        //   console.log("test");
          ...
          this.setState({likeImg:likeImg});
              ...
              if(res.data.sharecount<1){
                var shareCount='Be first to like Asset';
            }else{
                var shareCount= res.data.sharecount;
...
<div class="likedSection mb-5">
    <h5><small class="titleCnt"><img class="shareA" src="../img/share.png" onClick={(ev) => {this.openSocial(ev);}} /> {this.state.shareCount}</small></h5>
    <div class="statusBar mb-2 hide" id="ico-social">
            {/* <a href="javascript:void(0);" onClick={(ev) => {this.sendslack(ev);}} title="Share by Slack">
                 <img src="../img/slack.png" class="mr-2 rounded-circle" />
             </a> */}
             <span class="mr-3 titleCnt">
                <a href="javascript:void(0);" onClick={(ev) => {this.sendMail(ev);}} title="Share by Email">
                    <img src="../img/email.png"  class="mr-2 rounded-circle bk-rounded"  />  
                          {/* <span class="mr-3 titleCnt">{this.state.shareCount}</span> */}
